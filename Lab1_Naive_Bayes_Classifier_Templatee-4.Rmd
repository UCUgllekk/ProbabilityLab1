...
tidy_text <- unnest_tokens(train, 'splitted', 'Message', token="words") %>% 
  filter(!splitted %in% stop_words) %>% count(splitted, sort=TRUE)
...
naiveBayes <- setRefClass("naiveBayes",
       ...
       methods = list(
                    initialize = function(dataFrame){
                      classCount <- table(dataFrame$Category)
                      classProb$0 <<- classCount[1]/nrow(dataFrame)
                      classProb$1 <<- classCount[2]/nrow(dataFrame)
                    },
