...
-   Roman Pavlosiuk підготовка, чистка даних, створення bag_of_words(), score() (f1, recall)
-   Ярема Тебенько - fit(), predict() score() (accuracy) :
-   Влад - вся візуалізаціяб висновок:
...
naiveBayes <- setRefClass("naiveBayes",
  fields = list(
    ...
    labels = "character"  # Include labels as a field
  ),
  methods = list(
    fit = function(X, y) {
      bagOfWords <<- X
    ...
    predict = function(message) {
      ...
      for (word in words) {
        if (word %in% featureProb$word) {
          ...
          log_prob_spam <- log_prob_spam*(word_probs$`1`+0.0000000001)
          log_prob_ham <- log_prob_ham*(word_probs$`0`+0.0000000001)
          
        }
        else{
          log_prob_ham <- log_prob_ham*0.0000000001
          log_prob_spam <- log_prob_spam*0.0000000001
        }
